...
if [[ -e "${ANSIBLE_KEY}.pub" && ! -e "$ANSIBLE_KEY" ]]; then
  echo "Ansible public key exists without its private key: ${ANSIBLE_KEY}.pub" >&2
  exit 1
fi

if [[ ! -e "$ANSIBLE_KEY" ]]; then
  ssh-keygen -q -t ed25519 -N '' -C 'ansible' -f "$ANSIBLE_KEY"
elif [[ ! -e "${ANSIBLE_KEY}.pub" ]]; then
  ssh-keygen -y -f "$ANSIBLE_KEY" >"${ANSIBLE_KEY}.pub"
fi
